...
func TestMergeRequestHandler(t *testing.T) {
	...
	t.Run("Should fetch merge requests", func(t *testing.T) {
		...
		svc := mergeRequestListerService{emptyProjectData, fakeMergeRequestLister{}}
	...
	t.Run("Handles error from Gitlab client", func(t *testing.T) {
		request := makeRequest(t, http.MethodPost, "/merge_requests", testListMergeRequestsRequest)
		svc := mergeRequestListerService{emptyProjectData, fakeMergeRequestLister{testBase: testBase{errFromGitlab: true}}}
	...
	t.Run("Handles non-200s from Gitlab client", func(t *testing.T) {
		request := makeRequest(t, http.MethodPost, "/merge_requests", testListMergeRequestsRequest)
		svc := mergeRequestListerService{emptyProjectData, fakeMergeRequestLister{testBase: testBase{status: http.StatusSeeOther}}}
		data := getFailData(t, svc, request)
	...
	t.Run("Should handle not having any merge requests with 404", func(t *testing.T) {
		request := makeRequest(t, http.MethodPost, "/merge_requests", testListMergeRequestsRequest)
		svc := mergeRequestListerService{emptyProjectData, fakeMergeRequestLister{emptyResponse: true}}
		data := getFailData(t, svc, request)
