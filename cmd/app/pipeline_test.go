...
func TestPipelineGetter(t *testing.T) {
	t.Run("Gets all pipeline jobs", func(t *testing.T) {
		...
		svc := pipelineService{testProjectData, fakePipelineManager{}}
	...
	t.Run("Handles errors from Gitlab client", func(t *testing.T) {
		request := makeRequest(t, http.MethodGet, "/pipeline", nil)
		svc := pipelineService{testProjectData, fakePipelineManager{testBase{errFromGitlab: true}}}
		data := getFailData(t, svc, request)
		checkErrorFromGitlab(t, data, "Failed to get latest pipeline for some-branch branch")
	})
	t.Run("Handles non-200s from Gitlab client", func(t *testing.T) {
		request := makeRequest(t, http.MethodGet, "/pipeline", nil)
		svc := pipelineService{testProjectData, fakePipelineManager{testBase: testBase{status: http.StatusSeeOther}}}
		data := getFailData(t, svc, request)
