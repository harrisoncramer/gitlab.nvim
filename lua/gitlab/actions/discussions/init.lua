...
M.reply = function(tree)
	...
	local layout = M.create_reply_layout(tree, discussion_id)
	layout:mount()
end

-- This function will send the reply to the Go API
M.send_reply = function(tree, text, discussion_id)
...
---This function sets up the layout and popups needed to create a reply.
---It also sets up the basic keybindings for switching between window panes.
---@param discussion_id string
---@param tree NuiTree
---@return NuiLayout
M.create_reply_layout = function(tree, discussion_id)
	local reply_popup = Popup(u.create_popup_state("Reply", state.settings.popup.comment))
	local draft_popup = Popup(u.create_box_popup_state("Draft", false))

	local internal_layout = Layout.Box({
		Layout.Box(reply_popup, { grow = 1 }),
	...
	miscellaneous.set_cycle_popups_keymaps({ reply_popup, draft_popup })
	state.set_popup_keymaps(draft_popup, function()
		local text = u.get_buffer_text(reply_popup.bufnr)
		M.send_reply(tree, text, discussion_id)
		vim.api.nvim_set_current_win(M.split.winid)
	end, miscellaneous.toggle_bool, popup_opts)

	state.set_popup_keymaps(reply_popup, function(text)
		M.send_reply(tree, text, discussion_id)
		vim.api.nvim_set_current_win(M.split.winid)
