...
local create_multiline_diagnostic = function(d_or_n)
  ...
  local start_line, end_line, _ =
    actions_common.get_line_numbers_for_range(first_note.position.old_line, first_note.position.new_line, line_range)
...
local create_commit_old_side_diagnostic = function(d_or_n)
  ...
  local start_line, end_line = line_range.start.old_line, line_range["end"].old_line
  -- A block ending on an added line has no old-side number there, and Gitlab reports 0.
  if end_line == nil or end_line < start_line then
    return create_diagnostic({ lnum = start_line - 1 }, d_or_n)
  end
  return create_diagnostic({ lnum = start_line - 1, end_lnum = end_line - 1 }, d_or_n)
end
