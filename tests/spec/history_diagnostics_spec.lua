...
---A note on a block that starts on a line the commit deletes and ends on one it adds, as
---the web UI allows. The added endpoint has no old-side number, and Gitlab reports it 0.
---@param id string
---@param commit_id string
---@param old_line integer Deleted line the block starts on
---@param new_line integer Added line the block ends on
---@param path string
local function make_deleted_to_added_discussion(id, commit_id, old_line, new_line, path)
  local discussion = make_discussion(id, commit_id, new_line, path)
  local position = discussion.notes[1].position
  position.line_range = {
    start = { old_line = old_line, new_line = 0, type = "old" },
    ["end"] = { old_line = 0, new_line = new_line, type = "new" },
  }
  return discussion
end
...
describe("indicators/diagnostics.place_commit_diagnostics", function()
  ...
  it("Marks the start of a block that ends on a line the commit adds", function()
    state.DISCUSSION_DATA = { discussions = { make_deleted_to_added_discussion("d1", "sha1", 3, 4, "f.lua") } }
    state.DRAFT_NOTES = {}
    ...
    assert.are.equal(1, #result)
    assert.are.equal(2, result[1].lnum)
    assert.are.equal(2, result[1].end_lnum)
  end)
